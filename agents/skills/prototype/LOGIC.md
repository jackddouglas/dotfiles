...
Put the actual logic (the bit that's answering the question) in a single `<script>` block written as a small, pure module whose behavior can be translated into the real codebase later. The whole prototype is still throwaway; portability keeps the decision clear without making this untested code production-ready.
...
- **Don't ship prototype code into production.** The page is optimized for being clicked through by hand. Carry the validated behavior forward and implement it with production tests and constraints.
