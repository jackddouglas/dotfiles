...
Once a variant has won, capture the answer (which variant and why), then handle the prototype as [SKILL.md](SKILL.md) describes. Reimplement the winning behavior with production constraints and tests; do not promote the prototype code unchanged:

- **Sub-shape A**: use the winner to guide the existing page; leave the losing variants and switcher in the throwaway worktree.
- **Sub-shape B**: use the winner to guide a real route; leave the throwaway route and switcher in the prototype worktree.

Keep the full set on a throwaway branch only when the user asks to preserve it or a tracked implementation needs it as a primary source. Otherwise remove the worktree after the decision is recorded.
