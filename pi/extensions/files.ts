/**
 * Vendored from mitsuhiko/agent-stuff at
 * 3c891a9640f80c271ccc666ab7a39f9811bc3fb6. See files.LICENSE.
 * Modified locally to use local diff editors.
 */
...
const openPath = async (pi: ExtensionAPI, ctx: ExtensionContext, target: FileEntry): Promise<void> => {
	...
	if (getEditableContent(target).allowed) {
		const status = await ctx.ui.custom<number | null>((tui, theme, _kb, done) => {
			const message = new Text(theme.fg("dim", "Opening file in Neovim..."));

			queueMicrotask(() => {
				let exitStatus: number | null = null;
				try {
					tui.stop();
					exitStatus = spawnSync("nvim", [target.resolvedPath], { stdio: "inherit" }).status;
				} catch {
					exitStatus = null;
				} finally {
					tui.start();
		...
		if (status !== 0) {
			ctx.ui.notify(`Failed to open ${target.displayPath} in Neovim`, "error");
		}
		return;
	}
...
const canUseDiffview = (gitRoot: string): boolean => {
	const result = spawnSync(
		...
		[
			...
			"NONE",
			"-c",
			"lua if not pcall(require, 'diffview') then vim.cmd('cquit 1') end",
			"-c",
			"qa",
		],
...
const openDiffInNvim = async (ctx: ExtensionContext, relativePath: string, gitRoot: string): Promise<boolean> => {
	const status = await ctx.ui.custom<number | null>((tui, theme, _kb, done) => {
		const message = new Text(theme.fg("dim", "Opening diff in Neovim..."));
		...
		queueMicrotask(() => {
			...
			try {
				...
				const result = spawnSync(
					"nvim",
					["-c", "lua require('diffview').open({'HEAD', '--', vim.env.PI_FILES_DIFF_PATH})"],
					{
						cwd: gitRoot,
						env: { ...process.env, PI_FILES_DIFF_PATH: relativePath },
						stdio: "inherit",
					},
				);
...
const openDiff = async (pi: ExtensionAPI, ctx: ExtensionContext, target: FileEntry, gitRoot: string | null): Promise<void> => {
	...
	if (canUseDiffview(gitRoot) && (await openDiffInNvim(ctx, relativePath, gitRoot))) {
		return;
	}

	await openDiffInZed(pi, ctx, target, gitRoot, relativePath);
};
...
const showFileSelector = async (
	...
	const selection = await ctx.ui.custom<string | null>((tui, theme, keybindings, done) => {
		...
		container.addChild(
			new Text(theme.fg("dim", "Type to filter • enter to select • shift+opt+d diff • esc to cancel"), 0, 0),
		);
		container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
...
export default function (pi: ExtensionAPI): void {
	...
	pi.registerCommand("diff", {
		description: "Browse files and open a Git diff",
		handler: async (_args, ctx) => {
			await runFileBrowser(pi, ctx);
		},
	});
